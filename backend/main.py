...
# ============================================
# PASSWORD HASHING  (bcrypt-free, Railway-safe)
# ============================================

def _hash_password(password: str) -> str:
    """
    SHA-256 + per-user salt stored alongside the hash.
    Format stored: "sha256:<salt>:<hash>"
    Not as strong as bcrypt but zero extra deps and safe for this stage.
    Upgrade to bcrypt when you add pip install bcrypt to requirements.
    """
...
def _check_password(password: str, stored: str) -> bool:
    """Verify password against stored hash. Also accepts legacy plain-text."""
    if stored.startswith("sha256:"):
        try:
            ...
            return False
    # Legacy plain-text comparison (migrates on next login via save)
...
def load_data() -> dict:
    ...
    # Field migration for old records
...
def save_data(data: dict):
    """Write with exclusive file lock — safe for concurrent requests."""
    with open(DATA_FILE, 'r+') as f:
...
def _get_session_username(data: dict, session_id: str) -> str:
    """
    Resolve session_id → username.
    Handles both new dict format and old plain-string legacy format.
    ...
    # Legacy plain string — still valid until it expires naturally
    return session
...
async def fetch_github_data(github_username: str) -> dict:
    """Fetch public GitHub profile — language bytes not just repo counts."""
    try:
        async with httpx.AsyncClient() as client:
            ...
            # Aggregate language bytes across repos (accurate, not just counts)
...
async def register(request: RegisterRequest):
    ...
    new_builder = {
        ...
        "password": _hash_password(request.password),   # ← hashed
        **github_data,
        "bio": bio,
...
async def login(request: LoginRequest):
    ...
    # Constant-time-ish: always check password even if user not found
    ...
    if builder and not builder['password'].startswith("sha256:"):
...
async def update_profile(request: UpdateProfileRequest):
    ...
    fields = ['building_style', 'interests', 'open_to', 'availability',
